...
class SyTimeUtil {
  ...
  static bool isToday(int? milliseconds, {bool isUtc = false, int? locMs}) {
    ...
    DateTime now;
    if (locMs != null) {
      now = SyTimeUtil.getDateTimeByMs(locMs);
    } else {
  ...
  static bool isWeek(int? ms, {bool isUtc = false, int? locMs}) {
    ...
    DateTime _now;
    if (locMs != null) {
      _now = SyTimeUtil.getDateTimeByMs(locMs, isUtc: isUtc);
    } else {
...
// 时间戳转时间
  ...
  static String dateChange(String? dateStr) {
    List<String> newDateList = dateStr?.split('_') ?? [];
    String newDateStr = "";
    for (int i = 0; i < newDateList.length; i++) {
      String time = newDateList[i];
      if (i == 0 || i == 1) {
        time = "${newDateList[i]}-";
      } else if (i == 2) {
        time = "${newDateList[i]} ";
